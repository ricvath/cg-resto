  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full border-b border-[#e9e1d2]/25 bg-[#0e0d0b]/40 text-[#e9e1d2] backdrop-blur-[8px]">
      <div className="mx-auto flex min-h-[82px] w-full max-w-[1024px] items-center gap-4 px-4">
        ...
        <Button className="hidden h-auto rounded-none bg-[#e9e1d2] px-3 py-2 text-sm uppercase text-[#0e0d0b] hover:bg-[#cbb895] lg:inline-flex" onClick={() => window.open(bookingUrl, "_blank")}>Reserve a table</Button>
      ...
      {open && (
        <div className="border-t border-white/10 bg-[#0e0d0b]/95 px-4 pb-6 lg:hidden">
          <nav className="grid" aria-label="Mobile navigation">
            {navItems.map(([label, href]) => <NavLink key={href} to={href} onClick={() => setOpen(false)} className="border-b border-white/10 py-4 text-lg">{label}</NavLink>)}
          </nav>
          <Button className="mt-5 w-full rounded-none bg-[#e9e1d2] text-[#0e0d0b]" onClick={() => window.open(bookingUrl, "_blank")}>Reserve a table</Button>
        </div>
      )}
    </header>
  );
  ...
  <footer className="bg-[#0e0d0b] px-4 py-12 text-sm text-[#635b4f]">
    <div className="mx-auto grid max-w-[1024px] gap-8 md:grid-cols-[1.2fr_1fr_1fr]">
      ...
      <div className="space-y-3 text-sm">
        ...
        <p className="text-[#e9e1d2]">Open from 9 AM till 1 AM</p>
        <p>Every day</p>
      </div>
      <div>
        ...
        <div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-xs">
          <a href="https://cg.diff.hu/privacy-policy/">Privacy Policy</a>
          <a href="https://cg.diff.hu/legal-warning/">Legal Warning</a>
          <a href="https://cg.diff.hu/cookies-policy/">Cookies Policy</a>
        </div>
      </div>
    </div>
    <p className="mx-auto mt-10 max-w-[1024px] border-t border-white/10 pt-6 text-xs">© Cortes Garden Marbella · {new Date().getFullYear()} · All Rights Reserved</p>
  </footer>
);
